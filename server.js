...
// ─────────────────────────────────────────────────────────────
async function belgeDerinAnalizEt({ metin, base64Gorsel, dil = 'tr' }) {
...
- Sadece JSON döndür, markdown kod bloğu kullanma.`;
  ...
  if (base64Gorsel) {
    kullaniciIcerik.push({ type: 'image', source: { type: 'base64', media_type: 'image/jpeg', data: base64Gorsel } });
  }
  ...
  try {
    ...
    const metinYaniti = veri.content?.[0]?.text || '';
    let ayrisik;
    try { ayrisik = JSON.parse(metinYaniti); }
    catch { return { hata: 'AI yanıtı JSON olarak ayrıştırılamadı', ham: metinYaniti }; }
